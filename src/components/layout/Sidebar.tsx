...
export default function Sidebar() {
  ...
  return (
    <aside className="relative z-20 h-full">
      ...
      >
        <div className="flex h-full flex-col p-4 group">
          <div className="flex flex-row items-center gap-3 border-b border-b-red-500 pb-4 text-base">
          ...
          <div className="flex h-full flex-col gap-2 overflow-y-scroll hover:overflow-y-auto pr-2 py-2 conversations-scrollbar">
            {conversations.map((conversation, idx) => {
              return (
                <button
                  key={idx}
                  className="shadow-inset flex flex-row items-center rounded-md p-4
                   text-left text-sm shadow-white/40 hover:bg-neutral-900"
                >
